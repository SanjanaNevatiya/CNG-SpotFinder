	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(pass.equals(GetterSetterfuel.getPassword())){
			 Connection con = ConnectDB1.dbcon();
	            PreparedStatement ps = con.prepareStatement( "update user set upassword=? WHERE uemail = ? AND password = ?");
	            ps.setString(1, newpass);
	            ps.setString(2, GetterSetterUser.getUemail());
	            ps.setString(3, GetterSetterUser.getUpassword());
	            ps.executeUpdate();
